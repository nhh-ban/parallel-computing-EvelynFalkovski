...
# Parallel Computation within the function
MTweedieTests <-  
  function(N,M,sig){ 
    Cores <- min(parallel::detectCores(), 2)
    cl <- makeCluster(Cores)
    registerDoParallel(cl)
    clusterExport(cl, c("N", "sig"))
    one_simulation <- function() {
      replicate(M, simTweedieTest(N)) < sig
    }
    results <- parLapply(cl, 1:M, function(i) one_simulation())
    percent <- sum(unlist(results))/M
    stopCluster(cl)
    return(percent)
  } 
